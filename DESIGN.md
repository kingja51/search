...
- **개인정보 마스킹은 색인 시점에 수행** (`MaskingUtil` — 주민번호·카드번호·휴대폰·이메일):
...
24. **개인정보 마스킹은 색인 시점** — MaskingUtil(주민·카드·휴대폰·이메일)로 title/body를 마스킹 후 색인. 색인 DB 미저장·검색 채널 차단·성능 무영향. 뷰어는 표시 시점 보조 마스킹. 패턴 변경 시 전체 재색인.
